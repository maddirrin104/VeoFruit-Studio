-- DB schema for teacher test (PostgreSQL)
...
ALTER TABLE video_projects
  ADD COLUMN IF NOT EXISTS content_tone VARCHAR(100);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'video_projects_user_id_fkey'
  ) THEN
    ALTER TABLE video_projects
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'video_generations_project_id_fkey'
  ) THEN
    ALTER TABLE video_generations
      ADD CONSTRAINT video_generations_project_id_fkey
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'video_scenes_generation_id_fkey'
  ) THEN
    ALTER TABLE video_scenes
      ADD CONSTRAINT video_scenes_generation_id_fkey
      FOREIGN KEY (generation_id) REFERENCES video_generations(id) ON DELETE CASCADE ON UPDATE CASCADE;
  END IF;
END $$;
